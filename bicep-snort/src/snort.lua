...
-- block to add all configurations of an additional config directory
local config_dir = os.getenv("SNORT_CONFIG_DIR")
local handle = io.popen("ls " .. config_dir .. "*.lua")  
for filename in handle:lines() do
    local base_name = filename:match("([^/]+)%.lua$")  
    if base_name then
        include(config_dir .. base_name .. '.lua')  
    end
end
handle:close()
