...
@pytest.mark.asyncio
@patch("src.models.snort.execute_command_async", new_callable=AsyncMock)
async def test_execute_network_analysis_command(mock_execute_command, ids: Snort):
    ...
    mock_execute_command.assert_called_once_with([
       "snort", "-c", ids.default_configuration_location, "-i", ids.tap_interface_name, "-R", ids.ruleset_location, "-l", ids.log_location, "--plugin-path", "/etc/snort/etc/so_rules/",  "--plugin-path", "/usr/local/snort/lib/snort/plugins/extra"
    ])
    assert pid == 555
...
async def test_execute_static_analysis_command(mock_execute_command, ids: Snort):
    ...
    mock_execute_command.assert_called_once_with([
        "snort","-c", ids.default_configuration_location, "-R", ids.ruleset_location,  "-r", dataset_path, "-l", ids.log_location, "--plugin-path", "/etc/snort/etc/so_rules/",  "--plugin-path", "/usr/local/snort/lib/snort/plugins/extra"
    ])
    assert pid == 777
